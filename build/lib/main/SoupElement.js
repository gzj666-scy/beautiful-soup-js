...
class SoupElement {
    ...
    get string() {
        ...
        if (cur && cur.contents) {
            if (cur.contents.length == 1) {
                ...
                if (!res || res.type == Type_1.SoupType.Tag)
                    return undefined;
                return res;
            }
            else {
                const newCur = cur.contents.filter(v => v.type == Type_1.SoupType.Text);
    ...
    append(item) {
        ...
        if (!(this.type == Type_1.SoupType.Tag)) {
    ...
    _prettify(indent, breakline, level = 0) {
        ...
        for (let i = 0; i < this.contents.length; ++i) {
            if (this.contents[i].type == Type_1.SoupType.Text) {
                ...
                if (this.contents[i].type == Type_1.SoupType.Comment) {
    ...
    getText(separator = '') {
        ...
        for (let i = 0; i < descendants.length; ++i) {
            if (descendants[i].type == Type_1.SoupType.Text) {
...
exports.SoupElement = SoupElement;
