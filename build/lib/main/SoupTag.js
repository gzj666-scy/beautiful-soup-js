...
exports.SoupTag = void 0;
const SoupComment_1 = require("./SoupComment");
const SoupDoctypeString_1 = require("./SoupDoctypeString");
const SoupElement_1 = require("./SoupElement");
const SoupStrainer_1 = require("./SoupStrainer");
const SoupString_1 = require("./SoupString");
const Type_1 = require("./Type");
class SoupTag extends SoupElement_1.SoupElement {
    ...
    transform(dom) {
        if (!dom)
            return null;
        if (dom.type === 'text') {
            return new SoupString_1.SoupString(dom.data, this);
        }
        else if (dom.type === 'comment') {
            return new SoupComment_1.SoupComment(dom.data, this);
        }
        else if (dom.type === 'directive') { 
            if (dom.name === '!DOCTYPE') {
                return new SoupDoctypeString_1.SoupDoctypeString(dom.data, this);
            }
        }
    ...
    findAll(data) {
        const results = [];
        const strainer = new SoupStrainer_1.SoupStrainer(data);
        const nodes = data.recursive == undefined || data.recursive == null ? this.descendants : this.contents;
        for (let i = 0; i < nodes.length; ++i) {
            if (nodes[i] instanceof SoupElement_1.SoupElement) {
    ...
    append(item) {
        ...
        if (item.type == Type_1.SoupType.Text && pre.type == Type_1.SoupType.Text) {
            pre.text += item.text;
            return;
        }
    ...
    insert(index, newElement) {
        ...
        if (newElement.type == Type_1.SoupType.Soup) {
        ...
        if (typeof newElement == 'string') {
            newElement = new SoupString_1.SoupString(newElement);
        }
